...
/bin/gzip -d -c mining_districts.csv.gz | /usr/bin/psql -d $DATABASE -c "\\copy mining_district (name,region,geom) FROM STDIN WITH DELIMITER '|';"
...
/bin/gzip -d -c utm.csv.gz | /usr/bin/psql -d $DATABASE -c "\\copy utm (zone,geom) FROM STDIN WITH (DELIMITER ',', NULL '');"
